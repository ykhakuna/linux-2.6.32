...
#define key_input_name "key input event"
...
static int __init buttons_init(void)
{
	...
	if(!buttons_input)
		return  -ENOMEM;
	buttons_input->name=key_input_name;
#if 0
	...
	for(i = 0;i<sizeof(buttons_pin)/sizeof(buttons_pin[0]);i++)
	{
		...
		request_irq(buttons_pin[i].irq,buttons_interrupt, IRQF_SHARED | IRQF_TRIGGER_RISING | IRQF_TRIGGER_FALLING,buttons_pin[i].name,(void *)&buttons_pin[i]);
	}
	return 0;
